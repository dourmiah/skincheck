...
1. At the end, go to the `mlflow` tracking server and find the test results there
    ...
    * On the ohter hand, load_time, test_loss appears as results on the web page (here, programming is required)
...
1. In the `ModelTrainer` class
    ...
    My recomendation : you can read example of ``train.py`` where methods from the template have been modified more or less heavily in `../architecture/02_train_code/03_inceptionV3/train.py` 
...
* Before to `run_experiement.ps1` make sur the image is built and available (you will get a message otherwise)
